...
export abstract class GameJoystick extends GameObject{
    ...
    #listeners = new Set<(key: Key, state: KeyState) => any>()
    ...
    onUpdate(callback: (key: Key, state: KeyState) => any){
    ...
    offUpdate(callback: (key:Key, state:KeyState)=>any){
        this.#listeners.delete(callback)
        return this
    }

    onKeyUpdate(key: Key, callback: (state: KeyState) =>any ){
        return this.onUpdate((k, state) => k === key && callback(state))
    }

    onKeyUp(key: Key, callback: () =>any ){
        return this.onKeyUpdate(key, (state) => state === KeyState.UP && callback())
    }

    onKeyDown(key: Key, callback: () =>any ){
        return this.onKeyUpdate(key, (state) => state === KeyState.DOWN && callback())
    }
